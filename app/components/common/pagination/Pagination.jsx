...
function Pagination({pageCount,page,changeStep}) {
 return (
...
<nav >
  ...
  {new Array(Math.floor(pageCount)).fill(1).map((_, index) => (
   <li key={index}  onClick={()=>{
     changeStep(index) 
     }}>
      <a  ariaCurrent={(page)===index&&"page"} className=" cursor-pointer z-10 flex items-center justify-center px-3 h-8 leading-tight text-blue-600 border border-blue-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{index+1}</a>
    </li>
))}
 <li  onClick={()=>{
     if(page!==pageCount)
